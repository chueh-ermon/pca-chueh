...
for i = 1:numTestBat
    ...
    for j = 1:forEvery:numCycles
        cycle = j+startAt;
        PCAtest_row = [PCAtest_row, batch_test(i).cycles(cycle).discharge_dQdVvsV.dQdV(1,:)];
    end
...
plot(Y_test_pred, bat_label_test, 'o')
legend('train', '45 degree line', 'test')
name = strcat('PredCycle_CurrCycle', string(startAt), '_', string(forEvery), '_', string(numCycles));
